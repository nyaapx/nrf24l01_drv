...
void hal_nrf_set_irq_mode(hal_nrf_irq_source_t int_source, bool irq_state)
{
    if (irq_state) {
        hal_nrf_write_reg(CONFIG,
                hal_nrf_read_reg(CONFIG) & ~SET_BIT(int_source));
    } else {
        hal_nrf_write_reg(CONFIG,
                hal_nrf_read_reg(CONFIG) | SET_BIT(int_source));
    }
...
void hal_nrf_open_pipe(hal_nrf_address_t pipe_num, bool auto_ack)
{
    ...
    case HAL_NRF_PIPE5:
        hal_nrf_write_reg(EN_RXADDR,
                hal_nrf_read_reg(EN_RXADDR) | SET_BIT(pipe_num));

        if (auto_ack)
            hal_nrf_write_reg(EN_AA,
                    hal_nrf_read_reg(EN_AA) | SET_BIT(pipe_num));
        else
            hal_nrf_write_reg(EN_AA,
                    hal_nrf_read_reg(EN_AA) & ~SET_BIT(pipe_num));
        break;
...
{
    hal_nrf_write_reg(RF_SETUP,
            (hal_nrf_read_reg(RF_SETUP) & ~((1 << RF_PWR1) | (1 << RF_PWR0)))
                    | (UINT8(power) << 1));
}
...
void hal_nrf_set_datarate(hal_nrf_datarate_t datarate)
{
    if (datarate == HAL_NRF_1MBPS) {
        hal_nrf_write_reg(RF_SETUP,
                (hal_nrf_read_reg(RF_SETUP) & ~(1 << RF_DR)));
    } else {
        hal_nrf_write_reg(RF_SETUP,
                (hal_nrf_read_reg(RF_SETUP) | (1 << RF_DR)));
    }
...
{
    return (hal_nrf_read_reg(RF_SETUP) & ((1 << RF_PWR1) | (1 << RF_PWR0)))
            >> RF_PWR0;
}
...
{
    nrf_hw_spi_tx_tx(WR_ACK_PLOAD | pipe, tx_pload, length);
}
...
uint8_t hal_nrf_read_rx_pl_w (void)
{
    ...
    nrf_hw_spi_exchange(tx_buf, rx_buf, 2);
    return rx_buf[1];
}
...
void hal_nrf_set_pll_mode(hal_nrf_pll_mode_t pll_mode)
{
    if (pll_mode == HAL_NRF_PLL_LOCK) {
        hal_nrf_write_reg(RF_SETUP,
                (hal_nrf_read_reg(RF_SETUP) | (1 << PLL_LOCK)));
    } else {
        hal_nrf_write_reg(RF_SETUP,
                (hal_nrf_read_reg(RF_SETUP) & ~(1 << PLL_LOCK)));
    }
...
{
    return (hal_nrf_pll_mode_t) ((hal_nrf_read_reg(RF_SETUP) & (1 << PLL_LOCK))
            >> PLL_LOCK);
}
...
{
    return (hal_nrf_lna_mode_t) ((hal_nrf_read_reg(RF_SETUP) & (1 << LNA_HCURR))
            >> LNA_HCURR);
}
...
{
    uint8_t tx_buf[2], rx_buf[2], retval;

    /*
     *  (register access) or
     *  (single byte cmd OR future command/register access)
     */
    if ((reg < WRITE_REG) ||
        (!(reg == FLUSH_TX) && !(reg == FLUSH_RX) && !(reg == REUSE_TX_PL)
                            && !(reg == NOP))) {

        tx_buf[0] = WRITE_REG + reg;        /* write register command */
        tx_buf[1] = value;                  /* reg value */
        nrf24l01_spi_exchange(tx_buf, rx_buf, 2);
        retval = rx_buf[0];                 /* return state */

    /* single byte L01 command */
    } else {
        tx_buf[0] = reg;
        nrf24l01_spi_exchange(tx_buf, rx_buf, 1);
        retval = rx_buf[0];
    }
    return retval;
}
...
uint16_t hal_nrf_read_multibyte_reg(uint8_t reg, uint8_t *pbuf)
{
    ...
    case HAL_NRF_TX:
        ...
        nrf_hw_spi_tx_rx(RX_ADDR_P0 + reg, pbuf, length);
        break;

    case HAL_NRF_RX_PLOAD:
        if ((reg = hal_nrf_get_rx_data_source()) < 7) {
            length = hal_nrf_read_rx_pl_w();
            nrf_hw_spi_tx_rx(RD_RX_PLOAD, pbuf, length);
        } else {
            length = 0;
        }
        break;
...
void hal_nrf_write_multibyte_reg(uint8_t reg, uint8_t *pbuf, uint8_t length)
{
    ...
    case HAL_NRF_TX:
        length = hal_nrf_get_address_width();
        nrf_hw_spi_tx_tx(WRITE_REG + RX_ADDR_P0 + reg, pbuf, length);
        break;

    case HAL_NRF_TX_PLOAD:
        nrf_hw_spi_tx_tx(WR_TX_PLOAD, pbuf, length);
        break;
    default:
        break;
    }
